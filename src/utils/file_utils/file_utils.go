...
func SaveFile(header *multipart.FileHeader, file multipart.File, propertyId string) (string, rest_errors.RestErr) {
...
func UpdateFile(header *multipart.FileHeader, file multipart.File, path string, propertyId string) (string, rest_errors.RestErr) {
	splittedPath := strings.Split(path, "/")
	fileName := splittedPath[len(splittedPath)-1]
	DeleteFile(fileName, propertyId)
	newFileName, err := SaveFile(header, file, propertyId)
	if err != nil {
		return "", err
	}
